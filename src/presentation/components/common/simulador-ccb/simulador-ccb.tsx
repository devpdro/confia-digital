...
import {
  ...
  type ParsedCCB,
  type ScheduleRow,
} from "./simulador-ccb.utils";
import S from "./simulador-ccb.module.scss";
...
// ── shared animation primitives (matched to Confia patterns) ─
...
const staggerContainer = (stagger = 0.08, delayChildren = 0.05) => ({
  initial: { opacity: 0 },
...
const SimuladorCcb = () => {
    ...
    if (pvN > 0 && nN > 0 && pmtN > 0 && venc1) {
      ...
      setSelected((prev) => {
        const filtered = new Set(
          Array.from(prev).filter((k) => k <= newRows.length),
        );
  ...
  const applyCCB = (d: ParsedCCB) => {
    ...
    if (d.nome || d.cpf) {
      setDocTag(
        `EMITENTE: ${d.nome || "—"}${d.cpf ? ` · CPF ${d.cpf}` : ""} · CREDOR: Confia Capital`,
      );
    }
    if (d.n) {
  ...
  return (
    <section id="ccb-simulador" className={S.section}>
      <div className={S.container}>
        ...
        >
          <motion.span {...fadeUp(0, 12)} className={S.eyebrow}>
            <Sparkles size={12} strokeWidth={2.2} />
            {eyebrow}
          </motion.span>
          <motion.h1 {...fadeUp(0.08, 18)} className={S.title}>
            Liquidação antecipada <em>&amp; saldo devedor</em>
          </motion.h1>
          <motion.p {...fadeUp(0.16, 18)} className={S.subtitle}>
          ...
          <motion.p {...fadeUp(0.24, 18)} className={S.docTag}>
            {docTag}
          </motion.p>
        </motion.header>
        ...
        <motion.div
          ...
          {...cardRise}
        >
          <motion.div className={S.cardHead} {...staggerContainer(0.05, 0.05)}>
            <motion.span className={S.cardIdx} {...fadeUp(0, 10)}>
              01
            </motion.span>
            <motion.h2 className={S.cardTitle} {...fadeUp(0.05, 10)}>
              Dados da operação
            </motion.h2>
            <motion.span className={S.cardNote} {...fadeUp(0.1, 10)}>
          ...
          >
            ...
            <motion.label
              ...
              className={`${S.btnUpload} ${dirty ? S.dirty : ""}`}
              {...itemRise}
            >
            ...
            <AnimatePresence>
              {importStatus.msg && (
                <motion.span
                  ...
                  exit={{ opacity: 0, y: -4 }}
                  transition={{ duration: 0.25, ease: "easeOut" }}
                  className={`${S.importStatus} ${
                    importStatus.cls ? S[importStatus.cls] : ""
                  }`}
                >
                  {importStatus.cls === "ok" && (
        ...
        <motion.div
          ...
          className={`${S.card} ${S.hero}`}
          {...cardRise}
        >
          <motion.div className={S.cardHead} {...staggerContainer(0.05, 0)}>
            <motion.span
              className={`${S.cardIdx} ${S.heroIdx}`}
              {...fadeUp(0, 10)}
            >
              02
            </motion.span>
            <motion.h2
              className={`${S.cardTitle} ${S.heroTitle}`}
              {...fadeUp(0.05, 10)}
            >
          ...
          >
            ...
            <motion.div {...itemRise} className={S.heroStat}>
              <div className={S.bigLabel}>
                <TrendingDown size={12} strokeWidth={2.2} /> Economia de juros
              </div>
              <div className={`${S.sVal} ${S.sEco}`}>
          ...
          <motion.div
            ...
            {...staggerContainer(0.06, 0.1)}
          >
            <motion.span {...fadeUp(0, 8)}>
              Parcelas quitadas: <b>{q.pagas}</b>
            </motion.span>
            <motion.span {...fadeUp(0.06, 8)}>
              Parcelas vincendas: <b>{q.vincendas}</b>
            </motion.span>
            <motion.span {...fadeUp(0.12, 8)}>
          ...
          {rows.length === 0 && (
            ...
            >
              ...
              <div>
                ...
                PDF da cédula automaticamente, ou preencha os campos da
                operação acima. As tabelas aparecem assim que houver valor
                financiado, taxa, nº de parcelas e 1º vencimento.
              </div>
            </motion.div>
          )}
        </AnimatePresence>
                  ...
                  >
                    {rows.map((rw) => (
                      ...
                      >
                        ...
                        <td className={S.muted}>{fmtDate(rw.venc)}</td>
                        <td className={S.money}>
                          {BRL.format(rw.juros)}
                        </td>
                        <td className={S.money}>{BRL.format(rw.amort)}</td>
                        <td className={S.money}>
                          {BRL.format(rw.parcela)}
                        </td>
                        <td className={S.money}>{BRL.format(rw.saldo)}</td>
                      </motion.tr>
                    ))}
                  </motion.tbody>
                  <tfoot>
        ...
        <AnimatePresence>
          {rows.length > 0 && (
            ...
            >
              ...
              <motion.div
                className={S.pickBar}
                {...staggerContainer(0.05, 0.05)}
              >
                <motion.div {...itemRise} className={S.pickField}>
                ...
                >
                  {[
                    ...
                    <motion.button
                      ...
                      className={`${S.btn} ${S.btnQuick}`}
                      onClick={() =>
                        handleQuick(
                          b.key as "last" | "last2" | "all" | "none",
                        )
                      }
                    >
                      {b.label}
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        tabIndex={-1}
                      />
                      <span className={S.piK}>
                        {String(rw.k).padStart(2, "0")}
                      </span>
                      <span className={S.piMeta}>
                        {fmtDate(rw.venc)}
              ...
              <motion.div
                className={S.pickSummary}
                {...staggerContainer(0.07, 0.05)}
              >
                <motion.div {...itemRise} className={S.psItem}>
...
const Field = ({
  ...
  return (
    <div className={`${S.field} ${readOnly ? S.readonly : ""}`}>
      <label htmlFor={id}>
        {label}
